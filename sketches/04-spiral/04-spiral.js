...
function draw() {
  ...
  rectMode(CENTER);
  background(Red, Green, Blu);
  noFill();
      //allign to center of window
      translate(x/2, y/2);

    // To create some kind of animation i'm adjusting the width and height every sequence
    for (var i=0; i<200; i++){
      //to rotate the rectangle push() and pop() is used
      push();
      //frameCount is the variable that holds the number of frames assigned.
      //If frameCount is not assigned in setup it will keep incrimenting starting 0 to infinity
      rotate(cos(frameCount + i * 1.5) * 100);
      // To change a number(in this case the frames) within a certain range map() is used
      // sin() / cos() are the values being re-mapped
      // -1 and 1 are the bounds for sin() and cos() which are being changed to 50 and 255
      var r = map(sin(frameCount), -1, 1, 1, 120);
      var g =  map(cos(frameCount/2), -1, 1, 50, 180);
      var b = map(sin(frameCount/4), -1, 1, 200, 255);
      ...
      //the 5th parameter decides the curvature the shape will have
      rect(0, 0 , 600 - 3 * i, 600 - 3 * i, 210 - i);
...
function keyPressed() {
  ...
  switch (
    ...
    case 82: //R (Next Sketch)
      window.location.href = "../05-matrix/05-matrix.html"; //redirect current page to sketch 1, a test sketch
      break;
    ...
    case 70: //F (Previous Sketch)
      window.location.href = "../03-mandala/03-mandala.html"; //~~~PLACEHOLDER, UPDATE~~~ redirect current page to sketch 0, the home menu
      break;
    ...
    //encoder 2
    case 53: //5 ()
      //To change the backgroung color

      break;
    case 54: //6 ()
      //To go back to the default background color
      break;
    case 67: //C ()
      break;
...
function drawHud() {
  ...
  //keys are drawn by drawKey method. Parameters: x positon, y position, label, state, fill, stroke
  drawKey(keyWidth * 8, height - keyWidth * 2, "", "", keyCream, keyGrey); //Key 10 ()
  drawKey(keyWidth * 9, height - keyWidth * 2, "Change Color", "", keyCream, keyGrey); //Key 11 () U
  drawKey(keyWidth * 10, height - keyWidth * 2, "Reset Color", "", keyCream, keyGrey); //Key 12 () I
  ...
  drawKnob(keyWidth * 5, height - keyWidth * 2, "Test", 0, "Two", keyCream, keyGrey); //Knob 0 ()
